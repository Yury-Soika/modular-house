...
export type CompletedProjectContent = {
  titleRu: string;
  titleEn: string;
  metaRu: string;
  metaEn: string;
...
export default function HomePageClient({
  ...
  heroImage = "/family-house/IMG_0640.JPG",
  completedContent,
  settings
}: {
  initialContent?: Record<Lang, Content>;
  heroImage?: string;
  completedContent?: CompletedProjectContent[];
  settings?: { phone: string; email: string; catalog: string };
}) {
  const [lang, setLang] = useState<Lang>("ru");
  const [gallery, setGallery] = useState<Gallery | null>(null);
  const [activeCompleted, setActiveCompleted] = useState(0);
  const trackRef = useRef<HTMLDivElement>(null);
  const copy = initialContent[lang];
  const contactPhone = settings?.phone || CONTACT_PHONE;
  const contactEmail = settings?.email || CONTACT_EMAIL;
  const telegramUrl = `https://t.me/${contactPhone}`;
  const viberUrl = `viber://chat?number=${encodeURIComponent(contactPhone)}`;
  ...
  const completedProjects = completedContent?.length
    ? completedContent.map((project) => ({
        title: lang === "ru" ? project.titleRu : project.titleEn,
        meta: lang === "ru" ? project.metaRu : project.metaEn,
        text: lang === "ru" ? project.textRu : project.textEn,
        quote: project.quote,
  ...
  const contactActions = [
    { label: lang === "ru" ? "Позвонить" : "Call us", detail: copy.common.phone, href: `tel:${contactPhone}`, icon: Phone },
    { label: "Email", detail: contactEmail, href: `mailto:${contactEmail}`, icon: Mail },
    { label: copy.common.telegram, detail: copy.common.phone, href: telegramUrl, icon: MessageCircle },
    { label: copy.common.viber, detail: copy.common.phone, href: viberUrl, icon: MessageCircle }
  ];
  ...
  const openCompleted = (src: string) => {
    const heroGallery = completedProjects[0]?.images?.length ? completedProjects[0].images : familyHouseImages;
    openGallery(heroGallery, Math.max(0, heroGallery.indexOf(src)));
  };
    ...
    <main id="home" className="overflow-hidden" lang={copy.meta.htmlLang}>
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-forest-900/10 bg-white/95 shadow-sm backdrop-blur">
        <div className="section-shell flex h-20 items-center justify-between gap-5 text-forest-950">
          ...
          <a className="hidden items-center gap-2 text-sm font-semibold text-forest-950 md:flex" href={`tel:${contactPhone}`}>
              <Phone size={16} />
              {copy.common.phone}
            </a>
            <button
      ...
      <section className="relative min-h-[760px] bg-forest-950 pt-20 text-white">
        <button
          ...
          onClick={() => openCompleted(heroImage)}
          type="button"
        ...
        <div className="section-shell grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <div className="mt-4 space-y-3 text-sm text-white/75">
              <a className="flex items-center gap-2 transition hover:text-white" href={`tel:${contactPhone}`}><Phone size={16} /> {copy.common.phone}</a>
              <a
